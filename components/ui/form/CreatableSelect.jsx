...
const CreatableSelect = ({
  ...
  const select = (opt) => {
    ...
    setItems(prev => prev.find(o => o.value === opt.value) ? prev : [...prev, opt])
    close()
  }
  ...
  const showCreate   = !!onCreate && trimmedQuery.length > 0 && !exactMatch && !loading
  ...
  const highlight = (text, q) => {
    ...
    return (
      <>
        {text.slice(0, idx)}
        <mark style={{ background: 'var(--color-primary-bg)', color: 'var(--color-primary)', borderRadius: 2, padding: '0 1px' }}>
  ...
  return (
    ...
    >
      ...
      >
        {open ? (
          ...
          selected ? (
            <div className="creatable-selected">
              {selected.icon && <span className="creatable-selected-icon">{selected.icon}</span>}
        ...
        {/* right controls */}
        <div className="creatable-controls" onClick={e => e.stopPropagation()}>
          {loading && (
          ...
          <i className={`fa-solid fa-chevron-${open ? 'up' : 'down'} creatable-chevron`} />
        </div>
      </div>
        ...
        <div className="creatable-dropdown">
          ...
          {loading && (
            <div className="creatable-state">
              <i className="fa-solid fa-circle-notch fa-spin" style={{ fontSize: 14, color: 'var(--color-text-muted)' }} />
          ...
          {!loading && items.length === 0 && !showCreate && (
            <div className="creatable-state">
              <i className="fa-regular fa-folder-open" style={{ fontSize: 18, color: 'var(--color-text-muted)' }} />
              <span>{trimmedQuery ? `${emptyText} untuk "${trimmedQuery}"` : emptyText}</span>
            </div>
          )}
          ...
          {!loading && items.length > 0 && (
            <ul className="creatable-list">
              {items.map((opt, i) => (
                ...
                >
                  {opt.icon && <span className="creatable-item-icon">{opt.icon}</span>}
          ...
          {showCreate && (
            <div className="creatable-create-wrap">
              {items.length > 0 && (
                <div className="creatable-divider" />
              )}
              <button
                type="button"
              ...
              >
                {creating ? (
                  <>
                    <i className="fa-solid fa-circle-notch fa-spin" style={{ fontSize: 13 }} />
